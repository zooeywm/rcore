...
/// then execute `ecall` to trigger Trap. After Trap, back to U-mode, will
/// execute the next instruction after `ecall`, and we can read return value
/// from corresponding registers. `x10~x17` represent for `a0~a7`
///
/// `x1` represent for `ra`
///
/// - `a0~a2` pass the arguments to S-mode, and `a0` saves the return from
///   syscall
/// - `a7` saves syscall ID
